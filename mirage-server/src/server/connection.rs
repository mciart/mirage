...
/// Represents a Mirage connection encapsulating authentication and IO.
#[derive(Clone)]
...
impl MirageConnection {
    /// Creates a new instance of the Mirage connection.
    ///
    /// ### Arguments
    ...
    /// Starts the tasks for this instance of Mirage connection.
    pub async fn run(self, egress_queue: Receiver<Bytes>) -> (Self, MirageError) {
        if self.username.is_none() {
            let client_address = self.connection.remote_address();
            return (
                self,
                MirageError::system(format!(
    ...
    pub fn username(&self) -> Result<&str> {
        self.username
            .as_deref()
            .ok_or(MirageError::system("Connection is unauthenticated"))
    }
