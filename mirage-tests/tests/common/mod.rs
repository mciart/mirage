...
pub fn client_config() -> ClientConfig {
    ClientConfig::from_path(Path::new("tests/static/client.toml"), "MIRAGE_").unwrap()
}

#[fixture]
pub fn server_config() -> ServerConfig {
    ServerConfig::from_path(Path::new("tests/static/server.toml"), "MIRAGE_").unwrap()
}
...
macro_rules! interface_impl_imports {
    () => {
        use bytes::BytesMut;
        use ipnet::IpNet;
        use mirage::network::{interface::InterfaceIO, packet::Packet};
        use std::net::IpAddr;
    };
}
