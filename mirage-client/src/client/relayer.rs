...
use futures::StreamExt; // For FuturesUnordered
...
impl ClientRelayer {
    ...
    pub async fn start_prism(
        ...
        let relayer_task = tokio::spawn(Self::run_stack_architecture(
            interface,
            config,
            server_ip,
        ));
    ...
    pub async fn wait_for_shutdown(self) -> Result<()> {
        self.relayer_task.await.map_err(|_| MirageError::system("Relayer task failed"))?
    }
    ...
    async fn run_stack_architecture(
        ...
        tasks.push(tokio::spawn(Self::bridge_reader(iface_reader, tun_tx_to_stack)));
            ...
            tokio::spawn(async move {
                ...
                match Self::establish_tunnel(&config, server_ip).await {
                    Ok((mut remote_reader, mut remote_writer)) => {
                        ...
                        // 3. Pipe Data
                        let (mut local_tx, mut local_rx) = (req.tx, req.rx);
                        
                        // Upstream: Client -> Server
                        let t1 = tokio::spawn(async move {
                            while let Some(data) = local_rx.recv().await {
                                use tokio::io::AsyncWriteExt;
                                if remote_writer.write_all(&data).await.is_err() { break; }
                            }
                        });
                        ...
                        let t2 = tokio::spawn(async move {
                            ...
                            while let Ok(n) = remote_reader.read(&mut buf).await {
                                if n == 0 { break; }
                                if local_tx.send(Bytes::copy_from_slice(&buf[..n])).await.is_err() { break; }
                            }
                        });
    ...
    async fn establish_tunnel(
        config: &ClientConfig,
        server_ip: std::net::SocketAddr,
    ) -> Result<(tokio::io::ReadHalf<tokio_boring::SslStream<TcpStream>>, tokio::io::WriteHalf<tokio_boring::SslStream<TcpStream>>)> {
        ...
        let tls_stream = tokio_boring::connect(ssl_config, sni, tcp_stream).await
