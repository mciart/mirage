...
impl IpcConnection {
    ...
    pub async fn recv(&mut self) -> Result<IpcMessage> {
        ...
        if line.is_empty() {
            return Err(MirageError::system("Connection closed"));
        }
    ...
    #[cfg(unix)]
    {
        ...
        if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
            ...
            if runtime_path.exists() && runtime_path.is_dir() {
                return runtime_path.join(format!("mirage-{instance_name}.sock"));
            }
        }
        // Fall back to temp_dir if XDG_RUNTIME_DIR is not available
        env::temp_dir().join(format!("mirage-{instance_name}.sock"))
    }
    ...
    #[cfg(unix)]
    {
        ...
        if let Ok(state_dir) = env::var("XDG_STATE_HOME") {
            ...
            if state_path.exists() && state_path.is_dir() {
                return state_path.join(format!("mirage-{instance_name}.log"));
            }
        }
        if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
            let runtime_path = PathBuf::from(&runtime_dir);
            if runtime_path.exists() && runtime_path.is_dir() {
                return runtime_path.join(format!("mirage-{instance_name}.log"));
            }
        }
        env::temp_dir().join(format!("mirage-{instance_name}.log"))
    }
