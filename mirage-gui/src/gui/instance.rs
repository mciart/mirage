...
use super::types::MirageInstance;
use crate::ipc::{
...
impl MirageInstance {
    ...
    /// Starts a new Mirage VPN client instance.
    ///
    ...
    ///
    /// # Returns
    /// * `Ok((MirageInstance, Option<ConnectionMetrics>))` if the instance started successfully
    ...
    pub async fn start(
        ...
        let connection = match timeout(Self::DAEMON_START_TIMEOUT, ipc_server.accept()).await {
            ...
            Err(_) => {
                ...
                return Err(MirageError::system(Self::get_spawn_error(handle)));
            }
        };
    ...
    fn get_daemon_binary_path() -> Result<PathBuf> {
        Ok(env::current_exe()?
            .parent()
            .ok_or_else(|| MirageError::system("Could not determine parent directory"))?
            .join("mirage-client-daemon"))
    }
    ...
    fn spawn_daemon_process(
        ...
        PrivilegedCommand::new(daemon_binary)
            ...
            .arg(log_str)
            .gui(true)
            .prompt("Mirage needs administrator privileges to create network interfaces.")
            .spawn()
            .map_err(|err| MirageError::system(format!("{err}")))
    }
    ...
    async fn fetch_initial_status(&self) -> Result<Option<ConnectionMetrics>> {
        ...
        match connection.recv().await? {
            IpcMessage::StatusUpdate(status) => match status.status {
                ...
                ConnectionStatus::Disconnected => Err(MirageError::system("Daemon disconnected")),
                ConnectionStatus::Error(err) => Err(MirageError::system(err.to_string())),
            },
            IpcMessage::Error(err) => Err(MirageError::system(err.to_string())),
            other => {
