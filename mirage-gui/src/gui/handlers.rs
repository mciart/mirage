...
impl MirageGui {
    ...
    pub fn handle_new_config(&mut self) -> Task<Message> {
        ...
        let template_content = include_str!("../../../examples/client.toml");
    ...
    pub fn handle_connect(&mut self) -> Task<Message> {
        ...
        Task::future(async move {
            ...
            match MirageInstance::start(config_name.clone(), config_path).await {
                Ok((instance, metrics)) => {
                    info!(
                ...
                Err(e) => {
                    error!("Failed to start Mirage instance: {}", e);
