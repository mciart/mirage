...
COPY --from=builder /tmp/mirage-build/target/release/mirage /usr/local/bin/

# Add a non-root user
RUN addgroup -S mirage && adduser -S mirage -G mirage
RUN chown -R mirage:mirage /usr/local/bin/mirage

# Add required capabilities to executables
RUN setcap \
    'cap_net_admin,cap_net_bind_service=+ep' /usr/local/bin/mirage \
